        ...
        <ul class="pagination">
            ...
            @if ($paginator->onFirstPage())
                <li class="page-item disabled" aria-disabled="true" aria-label="@lang('pagination.previous')">
                    <span class="item-pagination flex-c-m trans-0-4" aria-hidden="true">&lsaquo;</span>
                </li>
            @else
                <li class="page-item">
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        @if ($page == $paginator->currentPage())
                            <li class="page-item" aria-current="page"><span class="item-pagination flex-c-m trans-0-4 active-pagination">{{ $page }}</span></li>
                        @else
                            <li class="page-item"><a class="item-pagination flex-c-m trans-0-4" href="{{ $url }}">{{ $page }}</a></li>
                        @endif
                    @endforeach
                @endif
            @endforeach
